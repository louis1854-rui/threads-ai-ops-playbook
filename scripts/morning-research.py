    ...
    def get_sheet(spreadsheet, keyword):
            for ws in spreadsheet.worksheets():
                ...
                def get_competitor_accounts():
                        c = gc()
                        if not c: return []
                                rows = get_sheet(c.open_by_key(SHEET_ID), "競合アカウントリスト").get_all_values()
    if not rows: return []
            h = rows[0]
...
def bl(d, u, h, m=""): c=gc(); c and get_sheet(c.open_by_key(SHEET_ID), "バズ投稿ログ").append_row([d,u,h,m])
...
def ml(d, u, h, m=""): c=gc(); c and get_sheet(c.open_by_key(SHEET_ID), "自分の投稿ログ").append_row([d,u,h,m])
